...
export function buildSkeleton(profile, weights, events, availability, focuses, weekHours) {
  ...
  const weeks = starts.map((ms, wi) => {
    ...
    const days = NAMES.map((name, di) => {
      ...
      if (pw) return { day: name, date, type: "ride", intensity: pw.intensity, title: pw.title, duration: fmtDur(stepsMins(pw.steps)), description: pw.description, steps: pw.steps };
      const s = S[key];

import { ask, askWithImage, chat, PLAN_MODEL } from "./anthropic.js";
...
ACTIONS: If — and only if — you are clearly recommending one of these specific changes, append it on its own line at the very end as a tag, and never mention the tag in your prose:
[[ease_week]] — make THIS week easier (turn the hard sessions into endurance).
[[rest_today]] — take today off entirely.
Use a tag only when you're genuinely recommending that change in your reply. Otherwise add no tag.`;
...
export function applyAvailability(block, availability) {
  ...
  for (const wk of block.weeks) {
    ...
    wk.days = (wk.days || []).map((d, di) => {
      ...
      return { day: d.day, type: "rest", intensity: "rest", title: a.type === "illness" ? "Illness — off" : "Holiday — off", duration: "—", description: a.notes || "No training — recover.", status: "off" };
    });
  }
  return block;
}

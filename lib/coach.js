...
- TODAY (${ctx?.today || "n/a"}) is scheduled: ${ctx?.todaySession || "n/a"}.
- Recent sessions:
${recentSessionsText(sessions, profile)}
...
[[PROPOSAL]]{"action":"set_day","date":"today|tomorrow|Tuesday|YYYY-MM-DD","to":"endurance|recovery|tempo|sweetspot|threshold|vo2|anaerobic|sprint|rest|easier|harder","summary":"..."}[[/PROPOSAL]] — for "change today's session to an easy ride", "make today endurance instead", "replace tomorrow's session with threshold", "make Thursday a rest day", "make today easier/harder". For the date you may simply write "today", "tomorrow", or the weekday name — the app resolves it, so you never need to calculate a date.

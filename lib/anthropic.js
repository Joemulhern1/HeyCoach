...
// Same, but with several images in one message — for reading multiple screenshots of ONE workout.
export async function askWithImages(prompt, images, maxTokens = 700, model = MODEL) {
  ...
  const res = await getClient().messages.create({ model, max_tokens: maxTokens, messages: [{ role: "user", content }] });
  return textFrom(res);
}
